...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Update {{ $category->name }} information</p>
        </div>
        <a href="{{ route('admin.categories.show', $category) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                ...
                <div>
                    ...
                    <select name="color" id="color" required
                        ...
                        @foreach($colors as $value => $name)
                            <option value="{{ $value }" {{ old('color', $category->color) === $value ? 'selected' : '' }}>
                                {{ $name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="icon" id="icon" required
                        ...
                        @foreach($icons as $iconName)
                            <option value="{{ $iconName }}" {{ old('icon', $category->icon) === $iconName ? 'selected' : '' }}>
